        ...
        <div class="container">
            ...
            <div>
                <div class="topbar-text dropdown d-md-none {{Session::get('direction') === "rtl" ? 'mr-auto' : 'ml-auto'}}">
                    <a class="topbar-link" href="tel: {{$web_config['phone']->value}}" title="رقم الهاتف">
                        <i class="fa fa-phone"></i> {{$web_config['phone']->value}}
                    </a>
                </div>
                <div class="d-none d-md-block {{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}} text-nowrap">
                    <a class="topbar-link d-none d-md-inline-block" href="tel:{{$web_config['phone']->value}}" title="رقم الهاتف">
                        <i class="fa fa-phone"></i> {{$web_config['phone']->value}}
                    </a>
                </div>
            </div>
            ...
            <div>
                ...
                <div
                    ...
                    <a class="topbar-link dropdown-toggle" href="#" data-toggle="dropdown" title="اختر اللغة">
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse"
                        ...
                        <li class="nav-item {{!request()->is('/')?'dropdown':''}}">
                            <a class="nav-link dropdown-toggle {{Session::get('direction') === "rtl" ? 'pr-0' : 'pl-0'}}"
                               href="#" data-toggle="dropdown" style="{{request()->is('/')?'pointer-events: none':''}}">
                                ...
                                </span> <!--عدل هنا ألوان الهوفر على التصنيغات-->
                            </a>
                            @if(request()->is('/'))
                                ...
                                box-shadow: none;min-width: 303px !important;{{Session::get('direction') === "rtl" ? 'margin-right: 1px!important;text-align: right;' : 'margin-left: 1px!important;text-align: left;'}}padding-bottom: 0px!important;">
                                    @foreach($categories as $key=>$category)
                                        ...
                                        onMouseOut="this.style.color='red'"
                                                ...
                                                "> -->
                                                        <span
                                                            class="{{Session::get('direction') === "rtl" ? 'pr-3' : 'pl-3'}}" style="font-weight: bold; font-size: 15px; " title="{{$category['name']}}">{{$category['name']}}</span>
                                                    </div>
                                                    @if ($category->childes->count() > 0)
                                                        <div>
                                                            <i class="czi-arrow-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}" style="
                                        ...
                                        "></i>
                                                ...
                                                @if($category->childes->count()>0)
                                                    <ul class="dropdown-menu"
                                                        style="right: 100%; width: 450px; hight: 50px; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                                ...
                                                ">
                                                                        <span
                                                                            class="{{Session::get('direction') === "rtl" ? 'pr-3' : 'pl-3'}}"style="font-weight: bold;" title="{{$subCategory['name']}}">{{$subCategory['name']}}</span>
                                                                    </div>
                                                                    @if ($subCategory->childes->count() > 0)
                                                                        <div>
                                                                            <i  class="czi-arrow-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}" style="font-size: 10px !important;
                                                                            background: #334257; !important;color:white;"></i>
                                                                        </div>
                                                                    @endif
                                                                </a>
                                                                @if($subCategory->childes->count()>0)
                                                                    <ul class="dropdown-menu"
                                                                        style="right: 100%; width: 400px; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                                                        @foreach($subCategory['childes'] as $subSubCategory)
                                                                            <li>
                                                                                <a class="dropdown-item"
                                                                                   href="{{route('products',['id'=> $subSubCategory['id'],'data_from'=>'category','page'=>1])}}" style="font-weight: bold; hight: 70px;" title="{{$subSubCategory['name']}}">{{$subSubCategory['name']}}</a>
                                                                            </li>
                                                                        @endforeach
                                                                    </ul>
                                                                @endif
                                    ...
                                    <a class="dropdown-item text-capitalize" href="{{route('categories')}}"
                                       style="color: {{$web_config['primary_color']}} !important; background:none; {{Session::get('direction') === "rtl" ? 'right' : 'left'}}: 29%">
                    ...
                    <ul class="navbar-nav" style="{{Session::get('direction') === "rtl" ? 'padding-right: 0px' : ''}}">
                        <li class="nav-item dropdown {{request()->is('/')?'active':''}}">
                            <a class="nav-link" href="{{route('home')}}" title="الصفحة الرئيسية">{{ \App\CPU\translate('Home')}}</a>
                        </li>

                        @if(\App\Model\BusinessSetting::where(['type'=>'product_brand'])->first()->value)
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#"
                               data-toggle="dropdown" title="العلامات التجارية">{{ \App\CPU\translate('brand') }}</a>
                            <ul class="dropdown-menu dropdown-menu-{{Session::get('direction') === "rtl" ? 'right' : 'left'}} scroll-bar"
                                ...
                                <li style="border-bottom: 1px solid #e3e9ef; display:flex; justify-content:center;">
                                    <div>
                                        ...
                                        style="color: {{$web_config['primary_color']}} !important;" title="عرض  المزيد">
                                            {{ \App\CPU\translate('View_more') }}
                                        </a>
                                    </div>
                                </li>
                        ...
                        @if ($discount_product>0)
                            <li class="nav-item dropdown {{request()->is('/')?'active':''}}">
                                <a class="nav-link text-capitalize" href="{{route('products',['data_from'=>'discounted','page'=>1])}}" title="المنتجات المخفضة">{{ \App\CPU\translate('discounted_products')}}</a>
                            </li>
                        @endif
                        <li class="nav-item dropdown {{request()->is('/')?'active':''}}">
                                <a class="nav-link text-capitalize" href="{{route('products',['data_from'=>'latest','page'=>1])}}" title="المنتجات الأخيرة">{{ \App\CPU\translate('latest_products')}}</a>
                            </li>
                            <li class="nav-item dropdown {{request()->is('/')?'active':''}}">
                                <a class="nav-link text-capitalize" href="{{route('products',['data_from'=>'best-selling','page'=>1])}}"title="المنتجات الأكثر مبيعا">{{ \App\CPU\translate('best_selling_product')}}</a>
                            </li>
                            <li class="nav-item dropdown {{request()->is('/')?'active':''}}">
                                <a class="nav-link text-capitalize" href="{{route('contacts')}}" title="تواصل معنا">{{ \App\CPU\translate('Contact Us')}}</a>
                            </li>
